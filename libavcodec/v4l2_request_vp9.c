...
static int v4l2_request_vp9_post_frames_ctx(AVCodecContext *avctx)
{
    ...
    if (s->s.h.profile != 0 || s->s.h.bpp != 8 ||
        s->ss_h != 1 || s->ss_v != 1) {
        av_log(avctx, AV_LOG_ERROR,
               "V4L2 VP9 decoder only supports profile 0, 8-bit 4:2:0\n");
        return AVERROR(ENOTSUP);
    }

    ret = ff_v4l2_request_query_control(avctx, &compressed_hdr);
    if (ret < 0) {
        av_log(avctx, AV_LOG_ERROR,
               "V4L2 VP9 decoder lacks the compressed-header control\n");
        return ret;
    }
...
static int v4l2_request_vp9_init(AVCodecContext *avctx)
{
    ...
    ctx->base.post_probe = v4l2_request_vp9_post_frames_ctx;
